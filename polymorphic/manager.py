...
class PolymorphicManager(models.Manager):
    ...
    def __init__(self, queryset_class=None, *args, **kwrags):
        # Up till polymorphic 0.4, the queryset class could be specified as
        # parameter to __init__.
        # However, this doesn't work for related managers which instantiate a
        # new version of this class.
        # Hence, for custom managers the new default is using the
        # 'queryset_class' attribute at class level instead.
        if queryset_class:
            warnings.warn(
                "Using PolymorphicManager(queryset_class=..) is deprecated; "
                "override the queryset_class attribute instead",
                DeprecationWarning
            )
    ...
    # The advantage of this method is that not yet known member functions of
    # derived querysets will be proxied as well.
    # We exclude any special functions (__) from this automatic proxying.
    #
    # NOTE: Fetching the queryset is done by calling self.all() here on
    # purpose.
    # By using .all(), the proper get_query_set()/get_queryset() will be used
    # for each Django version.
    # Django 1.4/1.5 need to use get_query_set(), because the RelatedManager
    # overrides that.
    ...
    def __unicode__(self):
        return '%s (PolymorphicManager) using %s' % (
            self.__class__.__name__, self.queryset_class.__name__
        )
