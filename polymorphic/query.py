...
# by the polymorphic queryset.iterator() implementation; we use the same chunk
# size as Django
try:
...
class PolymorphicQuerySet(QuerySet):
    ...
    def _clone(self, *args, **kwargs):
        """Django's _clone only copies its own variables, so we need to copy ours
        here"""
    ...
    def instance_of(self, *args):
        """Filter the queryset to only include the classes in args (and their
        subclasses).
    ...
    def _filter_or_exclude(self, negate, *args, **kwargs):
        """We override this internal Django functon as it is used for all
        filter member functions."""
        # the Q objects
        translate_polymorphic_filter_definitions_in_args(self.model, args)
        ...
        return super(PolymorphicQuerySet, self)._filter_or_exclude(
            negate, *(list(args) + additional_args), **kwargs
        )

    def order_by(self, *args, **kwargs):
        """translate the field paths in the args, then call vanilla
        order_by."""
        new_args = [
    ...
    def _process_aggregate_args(self, args, kwargs):
        """for aggregate and annotate kwargs: allow ModelX___field syntax for
        kwargs, forbid it for args.
        Modifies kwargs if needed (these are Aggregate objects, we translate
        the lookup member variable)"""

        def patch_lookup(a):
            if django.VERSION < (1, 8):
                a.lookup = translate_polymorphic_field_path(
                    self.model,
                    a.lookup
                )
            else:
                # With Django > 1.8, the field on which the aggregate operates
                # is stored inside a query expression.
                if hasattr(a, 'source_expressions'):
                    a.source_expressions[0].name = \
        ...
        def get_lookup(a):
            if django.VERSION < (1, 8):
                ...
                return a.source_expressions[0].name

        for a in args:
            assert '___' not in get_lookup(a), \
                'PolymorphicModel: annotate()/aggregate(): ___ model ' + \
                'lookup supported for keyword arguments only'
        for a in six.itervalues(kwargs):
            patch_lookup(a)

    def annotate(self, *args, **kwargs):
        """translate the polymorphic field paths in the kwargs, then call
        vanilla annotate.
        _get_real_instances will do the rest of the job after executing the
        query."""
    ...
    def aggregate(self, *args, **kwargs):
        """translate the polymorphic field paths in the kwargs, then call
        vanilla aggregate.
        We need no polymorphic object retrieval for aggregate => switch it
        off."""
        self._process_aggregate_args(args, kwargs)
    ...
    if django.VERSION >= (1, 9):
        # On Django < 1.9, 'qs.values(...)' returned a new special
        # ValuesQuerySet object, which our polymorphic modifications didn't
        # apply to.
        # Starting with Django 1.9, the copy returned by 'qs.values(...)' has
        # the same class as 'qs', so our polymorphic modifications would apply.
        # We want to leave values queries untouched, so we set
        # 'polymorphic_disabled'.
    ...
    def _get_real_instances(self, base_result_objects):
        ...
        base class query. The class of all of them is self.model (our base
        model).

        Some, many or all of these objects were not created and stored as class
        self.model, but as a class derived from self.model. We want to re-fetch
        these objects from the db as their original class so we can return them
        just as they were created/saved.

        We identify these objects by looking at o.polymorphic_ctype, which
        specifies the real class of these objects (the class at the time they
        were saved).

        First, we sort the result objects in base_result_objects for their
        subclass (from o.polymorphic_ctype), and then we execute one db query
        per subclass of objects. Here, we handle any annotations from
        annotate().
        ...
        results = {}  # polymorphic dict of result-objects, keyed with their id
        ...
        # custom fields in derived objects (unclear yet who to put the blame
        # on).
        ...
        # We might assume that self.model._meta.pk.name gives us the name of
        # the primary key field, but it doesn't. Therefore we use
        # polymorphic_primary_key_name, which we set up in base.py.
        pk_name = self.model.polymorphic_primary_key_name

        # - sort base_result_object ids into idlist_per_model lists, depending
        #   on their real class;
        ...
        self_model_class_id = ContentType.objects.get_for_model(
            self.model, for_concrete_model=False
        ).pk
        self_concrete_model_class_id = ContentType.objects.get_for_model(
        ...
        for base_object in base_result_objects:
            ...
            # check if id of the result object occurres more than once - this
            # can happen e.g. with base_objects.extra(tables=...)
            if base_object.pk not in base_result_objects_by_id:
                ...
                if base_object.polymorphic_ctype_id == self_model_class_id:
                    # Real class is exactly the same as base class, go straight
                    # to results:
                    results[base_object.pk] = base_object

                else:
                    real_concrete_class = base_object.get_real_instance_class()
                    real_concrete_class_id = base_object \
                        .get_real_concrete_instance_class_id()

                    if real_concrete_class_id is None:
                        # Dealing with a stale content type
                        continue
                    elif real_concrete_class_id == self_concrete_model_class_id:  # noqa
                        # Real and base classes share the same concrete
                        # ancestor, upcast it and put it in the results
                        results[base_object.pk] = transmogrify(
                            real_concrete_class, base_object
                        )
                    else:
                        real_concrete_class = ContentType.objects.get_for_id(
        ...
        # For each model in "idlist_per_model" request its objects (the real
        # model) from the db and store them in results[].
        # Then we copy the annotate fields from the base objects to the real
        # objects.
        # Then we copy the extra() select fields from the base objects to the
        # real objects.
        ...
        for real_concrete_class, idlist in idlist_per_model.items():
            ...
            for real_object in real_objects:
                ...
                if _query_annotations(self.query):
                    for anno_field_name in six.iterkeys(
                        _query_annotations(self.query)
                    ):
                        attr = getattr(
                            base_result_objects_by_id[o_pk],
                            anno_field_name
                        )
                        setattr(real_object, anno_field_name, attr)

                if self.query.extra_select:
                    for select_field_name in six.iterkeys(
        ...
        # set polymorphic_annotate_names in all objects (currently just used
        # for debugging/printing)
        if _query_annotations(self.query):
            # get annotate field list
            annotate_names = list(six.iterkeys(_query_annotations(self.query)))
            for real_object in resultlist:
                real_object.polymorphic_annotate_names = annotate_names

        # set polymorphic_extra_select_names in all objects (currently just
        # used for debugging/printing)
        if self.query.extra_select:
            # get extra select field list:
            extra_select_names = list(six.iterkeys(self.query.extra_select))
            for real_object in resultlist:
    ...
    def get_real_instances(self, base_result_objects=None):
        """same as _get_real_instances, but make sure that __repr__ for
        ShowField... creates correct output"""
