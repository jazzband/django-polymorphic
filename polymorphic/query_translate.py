...
def translate_polymorphic_filter_definitions_in_kwargs(queryset_model, kwargs):
    ...
    Any kwargs with special polymorphic functionality are replaced in the
    kwargs dict with their vanilla django equivalents.

    For some kwargs a direct replacement is not possible, as a Q object is
    needed instead to implement the required functionality. In these cases the
    kwarg is deleted from the kwargs dict and a Q object is added to the return
    list.

    Modifies: kwargs dict
    Returns: a list of non-keyword-arguments (Q objects) to be added to the
    filter() query.
    """
    additional_args = []
...
def translate_polymorphic_Q_object(queryset_model, potential_q_object):
    def tree_node_correct_field_specs(my_model, node):
        ...
        for i in range(len(node.children)):
            ...
            if type(child) == tuple:
                # this Q object child is a tuple => a kwarg like
                # Q(instance_of=ModelB)
                key, val = child
                new_expr = _translate_polymorphic_filter_definition(
                    my_model, key, val
                )
...
def _translate_polymorphic_filter_definition(queryset_model, field_path,
                                             field_val):
    """
...
def translate_polymorphic_field_path(queryset_model, field_path):
    ...
    if not sep:
        return field_path
    assert classname, 'PolymorphicModel: %s: bad field specification' % \
        field_path
    ...
    if '__' in classname:
        # the user has app label prepended to class name via __ => use Django's
        # get_model function
        ...
        assert model, 'PolymorphicModel: model %s (in app %s) not found!' % (
            model.__name__, appname
        )
        if not issubclass(model, queryset_model):
            e = 'PolymorphicModel: queryset filter error: "' + \
                model.__name__ + '" is not derived from "' + \
        ...
        # Test whether it's actually a regular relation__ _fieldname (the field
        # starting with an _) so no tripple ClassName___field was intended.
        try:
            # rel = (field_object, model, direct, m2m)
            field = queryset_model._meta.get_field(classname)
            if isinstance(field, RelatedObject):
                # Can also test whether the field exists in the related object
                # to avoid ambiguity between class names and field names, but
                # that never happens when your class names are in CamelCase.
        ...
        def add_all_sub_models(model, result):
            if issubclass(model, models.Model) and model != models.Model:
                # model name is occurring twice in submodel inheritance tree =>
                # Error
                if model.__name__ in result and \
                   model != result[model.__name__]:
                    e = 'PolymorphicModel: model name alone is ambiguous: ' + \
                        '%s.%s and %s.%s!\n'
                    e += 'In this case, please use the syntax: ' + \
                        'applabel__ModelName___field'
                    assert model, e % (
        ...
        assert(
            model,
            'PolymorphicModel: model %s not found (not a subclass of %s)!' %
            (classname, queryset_model.__name__)
        )

    # create new field path for expressions, e.g. for baseclass=ModelA,
    # myclass=ModelC
...
def _create_model_filter_Q(modellist, not_instance_of=False):
    ...
    if type(modellist) != list and type(modellist) != tuple:
        if issubclass(modellist, PolymorphicModel):
            ...
            assert(
                False,
                'PolymorphicModel: instance_of expects a list of '
                '(polymorphic) models or a single (polymorphic) model'
            )

    def q_class_with_subclasses(model):
        q = Q(polymorphic_ctype=ContentType.objects.get_for_model(
