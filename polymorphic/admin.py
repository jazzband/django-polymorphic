...
class PolymorphicProxyModelChangeLogMixin(object):
    """
    This mixin fixes the admin changelog entries generated by proxied,
    polymorphic models. These are essentially the same as the vanilla
    superclass methods from admin.ModelAdmin except that the ContentType
    get_for_model calls now use the for_concrete_model=False argument.

    One final hurdle to polymorphic objects playing nice with the admin
    changelog is this issue: https://code.djangoproject.com/ticket/20121
    Without that patch, the LogEntry urls are of the form
    {{app_name}}/{{child_model_name}}/{{ id }}/
    instead of
    {{app_name}}/{{parent_model_name}}/{{ id }}/
    """
...
class PolymorphicParentModelAdmin(PolymorphicProxyModelChangeLogMixin, admin.ModelAdmin):
    """
...
class PolymorphicChildModelAdmin(PolymorphicProxyModelChangeLogMixin, admin.ModelAdmin):
    """
