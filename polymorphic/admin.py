...
class PolymorphicModelChoiceForm(forms.Form):
    """
    The default form for the ``add_type_form``. Can be overwritten and
    replaced.
    """
    ...
    def queryset(self, request, queryset):
        ...
        if value:
            ...
            raise PermissionDenied(
                ...
                'It must be registered as child model.'.format(value)
            )
        return queryset
...
class PolymorphicParentModelAdmin(admin.ModelAdmin):
    ...
    * :func:`get_child_models` should return a list of (Model, ModelAdmin)
      tuples
    * optionally, :func:`get_child_type_choices` can be overwritten to refine
      the choices for the add dialog.

    This class needs to be inherited by the model admin base class that is
    registered in the site.
    The derived models should *not* register the ModelAdmin, but instead it
    should be returned by :func:`get_child_models`.
    """
    ...
    #: Whether the list should be polymorphic too, leave to ``False`` to
    # optimize
    ...
    #: If your primary key consists of string values, update this regular
    #: expression.
    pk_regex = '(\d+|__fk__)'
    ...
    def __init__(self, model, admin_site, *args, **kwargs):
        ...
        self._child_admin_site = self.admin_site.__class__(
    ...
    def _lazy_setup(self):
        ...
        # By not having this in __init__() there is less stress on import
        # dependencies as well,
        # considering an advanced use cases where a plugin system scans for the
        # child models.
        ...
        # This is needed to deal with the improved ForeignKeyRawIdWidget in
        # Django 1.4 and perhaps other widgets too.
        # The ForeignKeyRawIdWidget checks whether the referenced model is
        # registered in the admin, otherwise it displays itself as a textfield.
        # As simple solution, just make sure all parent admin models are also
        # know in the child admin site.
        # This should be done after all parent models are registered off
        # course.
    ...
    def register_child(self, model, model_admin):
        ...
        """
        # After the get_urls() is called, the URLs of the child model can't be
        # exposed anymore to the Django URLconf, which also means that a "Save
        # and continue editing" button won't work.
        if self._is_setup:
        ...
        if not issubclass(model, self.base_model):
            raise TypeError("{0} should be a subclass of {1}".format(
    ...
    def get_child_models(self):
        ...
        if self.child_models is None:
            raise NotImplementedError(
                "Implement get_child_models() or child_models"
            )
    ...
    def get_child_type_choices(self, request, action):
        """
        Return a list of polymorphic types for which the user has the
        permission to perform the given action.
        """
        choices = []
        for model, __ in self.get_child_models():
    ...
    def _get_real_admin_by_ct(self, ct_id):
        ...
        if not model_class:
            raise Http404("No model found for '{0}.{1}'.".format(
                *ct.natural_key())
    ...
    def _get_real_admin_by_model(self, model_class):
        # In case of a ?ct_id=### parameter, the view is already checked for
        # permissions.
        # Hence, make sure this is a derived object, or risk exposing other
        # admin interfaces.
        if model_class not in self._child_models:
            raise PermissionDenied(
                "Invalid model '{0}', it must be registered as child model."
                .format(model_class)
            )
        ...
        try:
            ...
            raise ChildAdminNotRegistered(
                "No child admin site was registered for a '{0}' model."
                .format(model_class)
            )
    ...
    def get_urls(self):
        ...
        # custom URLs to be added to the end. This is done by adding '/$' to
        # the end of the regex.  The url needs to be recreated, patching
        # url.regex is not an option Django 1.4's LocaleRegexProvider changed
        # it.
        ...
        custom_urls = [
            url(
                r'^(?P<path>.+)$',
                self.admin_site.admin_view(self.subclass_view)
            )
        ]
        ...
        # Add reverse names for all polymorphic models, so the delete button
        # and "save and add" just work.
        # These definitions are masked by the definition above, since it needs
        # special handling (and a ct_id parameter).
        dummy_urls = []
        for model, __ in self.get_child_models():
...
class PolymorphicChildModelAdmin(admin.ModelAdmin):
    ...
    * It allows to set ``base_form`` so the derived class will automatically
      include other fields in the form.
    * It allows to set ``base_fieldsets`` so the derived class will
      automatically display any extra fields.
    ...
    def get_form(self, request, obj=None, **kwargs):
        # The django admin validation requires the form to have a 'class Meta:
        # model = ..' attribute, or it will complain that the fields are
        # missing.
        # However, this enforces all derived ModelAdmin classes to redefine the
        # model as well, because they need to explicitly set the model again -
        # it will stick with the base model.
        #
        # Instead, pass the form unchecked here, because the standard ModelForm
        # will just work.
        ...
        return super(PolymorphicChildModelAdmin, self).get_form(
            request, obj, **kwargs
        )
    ...
    def get_fieldsets(self, request, obj=None):
        ...
        if (hasattr(self, 'declared_fieldset') and self.declared_fieldsets) \
           or not self.base_fieldsets:
            return super(PolymorphicChildModelAdmin, self).get_fieldsets(
                request, obj
            )
    ...
    def get_subclass_fields(self, request, obj=None):
        ...
        subclass_fields = list(six.iterkeys(form.base_fields)) + \
        ...
        for fieldset in self.base_fieldsets:
            for field in fieldset[1]['fields']:
                try:
                    ...
                    # field not found in form, Django will raise exception
                    # later:
                    pass
